...
class TestExpandMaiyamok(unittest.TestCase):
    ...
    # --- Issue #4: a bare ๆ (nothing valid to repeat) is preserved, not deleted ---

    def test_maiyamok_bare_at_start_is_preserved(self):
        """A ๆ with no preceding Thai word to repeat is kept verbatim."""
        self.assertEqual(expand_maiyamok("ๆ นี่คือไม้ยมก"), "ๆ นี่คือไม้ยมก")

    def test_maiyamok_bare_among_punctuation_is_preserved(self):
        self.assertEqual(expand_maiyamok("!!! ๆ !!!"), "!!! ๆ !!!")

    def test_maiyamok_bare_alone_is_preserved(self):
        self.assertEqual(expand_maiyamok("ๆ"), "ๆ")

    def test_maiyamok_after_non_thai_is_preserved(self):
        """A ๆ following only non-Thai characters has nothing to repeat; keep it."""
        self.assertEqual(expand_maiyamok("abc ๆ xyz"), "abc ๆ xyz")

    def test_maiyamok_unbalanced_opener_bare_is_preserved(self):
